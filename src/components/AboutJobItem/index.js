...
class AboutJobItem extends Component {
  ...
  getJobDetails = async props => {
    ...
    const response = await fetch(apiUrl, optionsJobData)
    if (response.ok === true) {
      const data = await response.json()
      const updatedJodDetails = [data.job_details].map(eachItem => ({
      ...
      this.setState({
        ...
        jobDataDetails: updatedJodDetails,
      })
    } else {
  ...
  renderFailure = () => (
    <div>
      ...
      <button type="button" onClick={this.onFailure}>
        Retry
      </button>
    </div>
  )
